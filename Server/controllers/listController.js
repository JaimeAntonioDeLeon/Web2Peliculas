...
const getListsUserMovieFilter = async (req, res) => {
  try {
    const { id, movie } = req.body;
    const lists = await List.find({ user_id: id, movies: { $ne: movie } });
    res.json(lists);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
...
const listAddMovie = async (req, res) => {
  try {
    const { id, movie } = req.body;
...
const listRemoveMovie = async (req, res) => {
  try {
    const { id, movie } = req.body;
    const post = await List.updateOne(
      { _id: id },
